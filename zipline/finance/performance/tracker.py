...
class PerformanceTracker(object):
    ...
    def __init__(self, sim_params):
        ...
        self._asset_expirations = {}
        self._dates_to_expire = set()
    ...
    def _update_asset(self, sid):
        ...
        if asset.end_date:
            try:
                date_sids = self._asset_expirations[asset.end_date]
                if asset.sid not in date_sids:
                    date_sids.append(asset.sid)
            except KeyError:
                date_sids = [asset.sid]
                self._asset_expirations[asset.end_date] = date_sids
    ...
    def check_upcoming_asset_ends(self, next_trading_day):
        """
        Checks for any assets that are ending the next day and closes them
        out by executing a transaction. The transaction is generated by the
        position_tracker.
        """
        for date, sids in self._asset_expirations.items():
            if date > next_trading_day:
                continue
            self._dates_to_expire.add(date)
            for sid in sids:
        ...
        while len(self._dates_to_expire) > 0:
            self._asset_expirations.pop(self._dates_to_expire.pop())
